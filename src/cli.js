...
function analyzeProject(cwd) {
  ...
  const checks = [
    ...
    check(
      ...
      "Add reviewer specs (.ai/reviewers/), tool-specific agents, or reviewer skills/plugins for high-risk areas.",
    ),
    check(
...
function countSkills(allFiles) {
  ...
  for (const f of allFiles) {
    ...
    if ((f.startsWith(".ai/workflows/") || f.includes("/.ai/workflows/")) && f.endsWith(".md")) n += 1;
    if ((f.startsWith("docs/workflows/") || f.includes("/docs/workflows/")) && f.endsWith(".md")) n += 1;
  }
  return n;
}
...
function hasPluginAgents(allFiles) {
  for (const file of allFiles) {
    ...
    // Codex plugins can package reviewer/agent definitions under their shipped
    // skill tree. Count those as specialist reviewer capability as well; this is
    // the plugin-distribution equivalent of `.claude/agents/`.
    if (/^skills\/[^/]+\/agents\/[^/]+\.(?:ya?ml|md)$/i.test(file)) return true;
  }
  return false;
}
...
function hasReviewerSkill(allFiles) {
  for (const file of allFiles) {
    ...
    if (/(^|\/)\.ai\/reviewers\/[^/]*(review|reviewer)[^/]*\.md$/i.test(file)) return true;
    if (/(^|\/)docs\/reviewers\/[^/]*(review|reviewer)[^/]*\.md$/i.test(file)) return true;
  }
  return false;
}
...
const GENERATED_AGENT_TOPS = new Set(["harness-reviewer"]);
...
function isUserAuthoredAgentPath(f) {
  const m = f.match(/(?:^|\/)\.claude\/agents\/(.+)$/i);
  if (!m || !m[1].endsWith(".md")) return false;
  const top = m[1].slice(0, -3);
  if (top.includes("/")) return true;
  return !GENERATED_AGENT_TOPS.has(top);
}
 ...
 *
 *  `vca init` may write Claude adapter commands only for Claude projects, and
 *  `evolve` writes a single project-evolution skill for any stack, so those
 *  generated artifacts are excluded: counting them would make a fresh non-Claude
 *  baseline fail its own newly-added hook checks on the next scan. The generated
 *  harness-reviewer agent is excluded for the same reason; other .claude/agents/
 *  entries remain user-authored signals. Any .claude/skills/ entry besides
 *  project-evolution is user-authored. */
...
Use .ai/workflows/ for reusable workflow prompts and .ai/reviewers/ for review
criteria. Keep durable rules in AGENTS.md so other AI tools share the same
source of truth.
`;
}
...
function cursorRule(name) {
  return `---
description: ${name} AI coding harness
alwaysApply: true
---
...
Use .ai/workflows/ for portable workflow prompts and .ai/reviewers/ for review
criteria. Keep durable rules in AGENTS.md; add Cursor-specific details here only
when they cannot be expressed in the shared source.
`;
}

function kiroSteering(name) {
  return `---
inclusion: always
---
...
Use .ai/workflows/ for portable workflow prompts and .ai/reviewers/ for review
criteria. Keep durable rules in AGENTS.md; add Kiro-specific steering here only
when it cannot be expressed in the shared source.
`;
}
...
function slashInitCommand() {
  return `Initialize a minimum viable AI coding harness for this repository.
...
function githubCiWorkflow(report) {
  const scripts = report.packageJson?.scripts || {};
  const run =
    scripts.ci ? "npm run ci" :
    scripts.validate ? "npm run validate" :
    scripts.test ? "npm test" :
    "npx --yes vibe-coding-analytics scan";
  return `name: CI
...
${name} needs stable project facts, repeatable validation, shared memory, review
...
function harnessReviewerAgent() {
  return `---
name: harness-reviewer
description: Review AI coding harness changes for missing sensors, unsafe agent rules, CI gaps, and weak feedback loops.
---

# Harness Reviewer

Review changes to agent instructions, CI, scripts, skills, commands, and project
memory.
...
- a repeated bug or user correction was fixed without adding a regression test,
  validator, rule, command, skill, reviewer, or decision record.

Prefer small, enforceable recommendations. Do not add project-specific business
rules to generic reusable skills.
`;
}

function workflowAnalyticsDoc() {
  return `# Analytics Workflow

Audit this repository for AI coding readiness.

Run or simulate:

\`\`\`bash
npx vibe-coding-analytics scan
\`\`\`

Return current maturity, missing harness areas, and now/next/later
recommendations. Prefer executable sensors over prose-only recommendations.
`;
}

function workflowInitDoc() {
  return `# Init Workflow

Initialize or refresh the minimum viable AI coding harness.
...
Review generated files, adapt commands to the project, then run validation.
Keep tool-specific adapters under their own directories only when that tool is
actually used by the project.
`;
}

function workflowEvolveDoc() {
  return `# Evolve Workflow

Improve the harness from recent repeated work.

Look for repeated failures, repeated user corrections, recurring commands,
review feedback, CI failures, and missing context. Promote each pattern into a
test, validator, rule, workflow, skill, reviewer, ADR, or scheduled check.

For optional CI mining:
...
function workflowSteerDoc() {
  return `# Steer Workflow
